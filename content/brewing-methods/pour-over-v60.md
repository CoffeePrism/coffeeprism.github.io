...
categories: ["冲泡方法"]
tags: ["V60", "手冲咖啡", "滴滤咖啡"]
description: "V60 手冲完全指南：60° 锥角原理、标准粉水比、四段式注水法、参数微调。一杯稳定好喝的手冲咖啡分步教程，附器材清单。"
howto:
  name: "V60 手冲咖啡标准做法"
  description: "用 Hario V60 滤杯冲一杯 240ml 手冲咖啡的标准流程，约 4 分钟。"
  totalTime: "PT4M"
  yield: "1 杯 240ml"
  supply:
    - "15g 浅至中焙阿拉比卡咖啡豆"
  ...
  steps:
    - name: "润湿滤纸 + 预热器具"
      text: "把滤纸放进 V60，用热水从上到下绕圈淋湿整张滤纸，同时预热滤杯和下方分享壶。倒掉预热水。"
    - name: "研磨 + 布粉"
      text: "把 15g 咖啡豆磨到中细度（类似砂糖），约 700 微米。倒进滤杯轻拍让粉床平整。"
    - name: "闷蒸（Bloom）"
      text: "倒入 30g 热水（约 2 倍粉重），均匀浸湿所有粉。等 30-45 秒让 CO₂ 排出。"
    - name: "第一段注水"
      text: "从中心向外绕圈注水到 120g（累计），15-20 秒注完。等液面下降到一半。"
    - name: "第二段注水"
      text: "继续从中心绕圈注水到 240g（累计），约 15 秒。注水柱保持稳定细致。"
    - name: "等待滴滤完成"
      text: "总冲煮时间控制在 2:30-3:30 之间。最后液面下降时取下滤杯即可。"
---
